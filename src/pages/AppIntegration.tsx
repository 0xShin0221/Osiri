...
export default function AppIntegrationPage() {
  ...
  const handleSlackConnect = async () => {
    ...
    }/functions/v1/slack-callback?lang_userId=${combinedParam}&returnUrl=${encodeURIComponent(
      returnUrl
    )}`;
    window.location.href = `https://slack.com/oauth/v2/authorize?client_id=${
      import.meta.env.VITE_SLACK_CLIENT_ID
    }&scope=channels:join,channels:read,chat:write,channels:manage&user_scope=chat:write,channels:read&redirect_uri=${encodeURIComponent(
      redirectUri
    )}`;
  };
