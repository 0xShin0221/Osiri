...
const testTemplateContent = () => {
  ...
  assert(
    enTemplate.subject.includes("Osiri"),
    'English subject should contain "Osiri"',
  );
  assert(
    enTemplate.html.includes("Welcome"),
    "English content should contain welcome message",
  );
  assert(
    enTemplate.html.includes("50%"),
    "English content should mention discount",
  );

  const jaTemplate = getEarlyAccessTemplate("ja");
  assert(
    jaTemplate.subject.includes("Osiri"),
    "日本語の件名にOsiriが含まれるべき",
  );
  assert(
    jaTemplate.html.includes("ようこそ"),
    "日本語の本文に歓迎メッセージが含まれるべき",
  );
  assert(
    jaTemplate.html.includes("50%オフ"),
    "日本語の本文に割引の説明が含まれるべき",
  );
};
