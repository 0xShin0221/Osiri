...
export class ContentScraper {
  ...
  private log(type: 'start' | 'success' | 'error', url: string, details?: string) {
    const timestamp = new Date().toISOString();
    console.log(`[${timestamp}] [Scraper] [${type.toUpperCase()}] ${url}${details ? ` - ${details}` : ''}`);
  }
  ...
  async scrape(url: string, options?: ScraperOptions): Promise<ServiceResponse<string>> {
    ...
    try {
      const loader = new PlaywrightWebBaseLoader(url, {
        ...
        gotoOptions: {
          timeout: options?.timeout ?? this.defaultOptions.timeout,
          waitUntil: options?.waitUntil ? waitUntilMapping[options.waitUntil] : 'domcontentloaded'
        }
      });
  ...
  async scrapeBatch(urls: string[], options?: ScraperOptions): Promise<ServiceResponse<string>[]> {
    ...
    for (let i = 0; i < urls.length; i += opts.batchSize) {
      ...
      console.log(`[Scraper] Processing batch ${i / opts.batchSize + 1}/${Math.ceil(urls.length / opts.batchSize)}`);
